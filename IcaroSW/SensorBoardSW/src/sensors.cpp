...
int8_t bme280_init()
{
    ...
    bme280.delay_us = main_bmi_delay;
    bme280.read = main_i2c_read;
    bme280.write = main_i2c_write;

    status = bme280_init(&bme280);
    ESP_LOGE("BME280", "Init status: %d", status);

    bme280_settings settings;

    bme280_get_sensor_settings(&settings, &bme280);
...
float sensors_pt100RtoT(float R)
{
    ...
    float interT = (Ti - Tj)/((float)(Pt100_table[i]-Pt100_table[j]))*(R-Pt100_table[j]) + Tj;
    return interT;
}
//31.25e-6, 0.207f, 3.237f, 1004.0f
float sensors_AdsToTemp(uint16_t val, float gainFsr, float vb, float vcc, float r1)
{
    // DeltaV measured by the sensor
    float dv = val * gainFsr;
    float Z = dv + vb;
    float R = (Z * r1) / (vcc - Z);

    // Calculate temperature in Celsius
    return sensors_pt100RtoT(R);
}

void sensors_getPt100(float &pt100_1, float &pt100_2)
{
    int32_t adcValue;
    // ADS1115 request differential conversion between AIN0 and AIN1
    ESP_ERROR_CHECK(ADS1115_request_diff_AIN0_AIN1());
    
    // Wait for conversion to complete
    while (!ADS1115_get_conversion_state())
        vTaskDelay(pdMS_TO_TICKS(5));

    adcValue = ADS1115_get_conversion();
    //ESP_LOGE("SENSORS", "PT100_1: %ld", adcValue);
    // Get the conversion value and convert to temperature
    sensorData.pt100_1 = sensors_AdsToTemp(
        adcValue,
        31.25e-6, 0.207f, 3.237f, 1004.0f);
    
    // ADS1115 request differential conversion between AIN0 and AIN1
    ESP_ERROR_CHECK(ADS1115_request_diff_AIN2_AIN3());
    ...
    adcValue = ADS1115_get_conversion();
    //ESP_LOGE("SENSORS", "PT100_2: %ld", adcValue);

    sensorData.pt100_2 = sensors_AdsToTemp(
        adcValue,
        31.25e-6, 0.207f, 3.237f, 1004.0f);
