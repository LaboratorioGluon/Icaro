...
void initPacket()
{
    ...
    int pos = 0;
    memcpy(&packetBuffer[pos], &f802header, sizeof(f802header));
...
Raw80211Link::Raw80211Link(mac_t src, mac_t dest) : src(src), dest(dest)
// , packetBuffer{0}
{
    initPacket();
}
...
size_t Raw80211Link::write(const char* data, size_t length)
{
    ...
    const size_t len = sizeof(packetBuffer);
    esp_err_t err = esp_wifi_80211_tx(WIFI_IF_STA, packetBuffer, len, false);
    if (err != ESP_OK) {
